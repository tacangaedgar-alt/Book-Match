...
IF OBJECT_ID('dbo.Valoraciones') IS NULL CREATE TABLE dbo.Valoraciones(UsuarioId int NOT NULL REFERENCES dbo.Usuarios(UsuarioId),LibroId int NOT NULL REFERENCES dbo.Libros(LibroId),Puntuacion tinyint NOT NULL CHECK(Puntuacion BETWEEN 1 AND 5),Fecha datetime2 NOT NULL DEFAULT sysdatetime(),CONSTRAINT PK_Valoraciones PRIMARY KEY(UsuarioId,LibroId));
...
BEGIN
 ...
 INSERT dbo.Usuarios(RolId,Nombre,Correo,PasswordHash,EsAutor,Activo,FechaRegistro) SELECT RolId,N'Sofía Reyes','sofia@example.com',HASHBYTES('SHA2_256',CONVERT(nvarchar(4000),N'password123')),1,0,'2024-05-19' FROM dbo.Roles WHERE Nombre=N'Usuario';
...
BEGIN
 ...
 INSERT dbo.Biblioteca(UsuarioId,LibroId,FechaAdquisicion,Leido) SELECT @u,LibroId,DATEADD(day,-LibroId,sysdatetime()),CASE WHEN Codigo='BM-003' THEN 1 ELSE 0 END FROM dbo.Libros WHERE Codigo IN('BM-003','BM-004','BM-006');
 INSERT dbo.Biblioteca(UsuarioId,LibroId) SELECT x.UsuarioId,l.LibroId FROM (VALUES(@carlos,'BM-001'),(@carlos,'BM-003'),(@andres,'BM-001'),(@andres,'BM-002'),(@isabella,'BM-001'),(@isabella,'BM-002'))x(UsuarioId,Codigo) JOIN dbo.Libros l ON l.Codigo=x.Codigo;
 INSERT dbo.Valoraciones(UsuarioId,LibroId,Puntuacion) SELECT x.UsuarioId,l.LibroId,x.Puntuacion FROM (VALUES(@u,'BM-003',5),(@u,'BM-004',5),(@u,'BM-006',4),(@carlos,'BM-001',5),(@carlos,'BM-003',4),(@andres,'BM-001',4),(@andres,'BM-002',5),(@isabella,'BM-001',5),(@isabella,'BM-002',4))x(UsuarioId,Codigo,Puntuacion) JOIN dbo.Libros l ON l.Codigo=x.Codigo;
 UPDATE l SET Valoracion=COALESCE((SELECT CAST(AVG(CAST(v.Puntuacion AS decimal(4,2))) AS decimal(3,2)) FROM dbo.Valoraciones v WHERE v.LibroId=l.LibroId),0) FROM dbo.Libros l;
 DECLARE @purchase int; INSERT dbo.Compras(Codigo,UsuarioId,Fecha,Total) VALUES('ORD-2024-001',@u,'2024-10-11',9.99);SET @purchase=SCOPE_IDENTITY();INSERT dbo.CompraDetalle(CompraId,LibroId,Precio) SELECT @purchase,LibroId,9.99 FROM dbo.Libros WHERE Codigo='BM-003';
END
GO
...
BEGIN SET NOCOUNT ON;SELECT l.LibroId,l.Codigo,l.Titulo,u.Nombre Autor,g.Nombre Genero,l.Idioma,l.Descripcion,l.Precio,l.Valoracion,l.PortadaUrl,l.Estado,l.Ventas,l.Descargas,CAST(0 AS bit) Leido,CAST(l.FechaPublicacion AS datetime2) Fecha,CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.Biblioteca b WHERE b.UsuarioId=@UsuarioId AND b.LibroId=l.LibroId) THEN 1 ELSE 0 END AS bit) EnBiblioteca,CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.Carrito c WHERE c.UsuarioId=@UsuarioId AND c.LibroId=l.LibroId) THEN 1 ELSE 0 END AS bit) EnCarrito,CAST(CASE WHEN l.AutorId=@UsuarioId THEN 1 ELSE 0 END AS bit) EsPropio,(SELECT COUNT(*) FROM dbo.Valoraciones v WHERE v.LibroId=l.LibroId) CantidadValoraciones FROM dbo.Libros l JOIN dbo.Usuarios u ON u.UsuarioId=l.AutorId JOIN dbo.Generos g ON g.GeneroId=l.GeneroId WHERE l.Estado=N'Publicado' AND (@Busqueda IS NULL OR l.Titulo LIKE '%'+@Busqueda+'%' OR u.Nombre LIKE '%'+@Busqueda+'%') AND (@Genero IS NULL OR g.Nombre=@Genero) AND (@Idioma IS NULL OR l.Idioma=@Idioma) AND (@TipoPrecio='all' OR @TipoPrecio='free' AND l.Precio=0 OR @TipoPrecio='paid' AND l.Precio>0) AND (@ValoracionMinima IS NULL OR l.Valoracion>=@ValoracionMinima) ORDER BY l.FechaPublicacion DESC;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Biblioteca_Listar @UsuarioId int,@Filtro varchar(12)='all' AS
BEGIN SET NOCOUNT ON;SELECT l.LibroId,l.Codigo,l.Titulo,u.Nombre Autor,g.Nombre Genero,l.Idioma,l.Descripcion,l.Precio,l.Valoracion,l.PortadaUrl,l.Estado,l.Ventas,l.Descargas,b.Leido,b.FechaAdquisicion Fecha,CAST(CASE WHEN NULLIF(l.RutaPdf,N'') IS NULL THEN 0 ELSE 1 END AS bit) TienePdf,(SELECT COUNT(*) FROM dbo.Valoraciones v WHERE v.LibroId=l.LibroId) CantidadValoraciones,COALESCE((SELECT CONVERT(int,v.Puntuacion) FROM dbo.Valoraciones v WHERE v.UsuarioId=@UsuarioId AND v.LibroId=l.LibroId),0) ValoracionUsuario FROM dbo.Biblioteca b JOIN dbo.Libros l ON l.LibroId=b.LibroId JOIN dbo.Usuarios u ON u.UsuarioId=l.AutorId JOIN dbo.Generos g ON g.GeneroId=l.GeneroId WHERE b.UsuarioId=@UsuarioId AND (@Filtro='all' OR @Filtro='read' AND b.Leido=1 OR @Filtro='pending' AND b.Leido=0) ORDER BY b.FechaAdquisicion DESC;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Biblioteca_Acceso @UsuarioId int,@LibroId int,@MarcarLeido bit=0 AS
BEGIN SET NOCOUNT ON;IF @MarcarLeido=1 UPDATE dbo.Biblioteca SET Leido=1 WHERE UsuarioId=@UsuarioId AND LibroId=@LibroId;SELECT l.LibroId,l.Titulo,u.Nombre Autor,l.RutaPdf FROM dbo.Biblioteca b JOIN dbo.Libros l ON l.LibroId=b.LibroId JOIN dbo.Usuarios u ON u.UsuarioId=l.AutorId WHERE b.UsuarioId=@UsuarioId AND b.LibroId=@LibroId;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Publicacion_Listar @UsuarioId int AS
BEGIN SET NOCOUNT ON;SELECT l.LibroId,l.Codigo,l.Titulo,u.Nombre Autor,g.Nombre Genero,l.Idioma,l.Descripcion,l.Precio,l.Valoracion,l.PortadaUrl,l.Estado,l.Ventas,l.Descargas,CAST(0 AS bit) Leido,CAST(l.FechaPublicacion AS datetime2) Fecha,CAST(CASE WHEN NULLIF(l.RutaPdf,N'') IS NULL THEN 0 ELSE 1 END AS bit) TienePdf FROM dbo.Libros l JOIN dbo.Usuarios u ON u.UsuarioId=l.AutorId JOIN dbo.Generos g ON g.GeneroId=l.GeneroId WHERE l.AutorId=@UsuarioId ORDER BY l.FechaPublicacion DESC;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Publicacion_Acceso @UsuarioId int,@LibroId int AS
BEGIN SET NOCOUNT ON;SELECT l.LibroId,l.Titulo,u.Nombre Autor,l.RutaPdf FROM dbo.Libros l JOIN dbo.Usuarios u ON u.UsuarioId=l.AutorId WHERE l.LibroId=@LibroId AND l.AutorId=@UsuarioId;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Libro_Publicar @UsuarioId int,@Titulo nvarchar(180),@Genero nvarchar(80),@Idioma nvarchar(40),@Precio decimal(10,2),@Descripcion nvarchar(2000),@Contenido nvarchar(max)=NULL,@RutaPdf nvarchar(500),@PortadaUrl nvarchar(500) AS
BEGIN SET NOCOUNT ON;IF NULLIF(@RutaPdf,N'') IS NULL OR NULLIF(@PortadaUrl,N'') IS NULL THROW 50007,'El PDF y la portada son obligatorios.',1;DECLARE @GeneroId int=(SELECT GeneroId FROM dbo.Generos WHERE Nombre=@Genero);IF @GeneroId IS NULL THROW 50001,'Género no válido.',1;DECLARE @next int=ISNULL((SELECT MAX(LibroId) FROM dbo.Libros),0)+1;INSERT dbo.Libros(Codigo,AutorId,GeneroId,Titulo,Idioma,Descripcion,Precio,Valoracion,PortadaUrl,RutaPdf,Contenido,Estado) VALUES(CONCAT('BM-',FORMAT(@next,'000')),@UsuarioId,@GeneroId,@Titulo,@Idioma,ISNULL(@Descripcion,''),@Precio,0,@PortadaUrl,@RutaPdf,@Contenido,N'Publicado');SELECT CONVERT(int,SCOPE_IDENTITY());END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Carrito_Agregar @UsuarioId int,@LibroId int AS BEGIN SET NOCOUNT ON;IF NOT EXISTS(SELECT 1 FROM dbo.Libros WHERE LibroId=@LibroId AND Estado=N'Publicado') BEGIN SELECT 'not_found';RETURN;END IF EXISTS(SELECT 1 FROM dbo.Libros WHERE LibroId=@LibroId AND AutorId=@UsuarioId) BEGIN SELECT 'own_book';RETURN;END IF EXISTS(SELECT 1 FROM dbo.Biblioteca WHERE UsuarioId=@UsuarioId AND LibroId=@LibroId) BEGIN SELECT 'owned';RETURN;END IF EXISTS(SELECT 1 FROM dbo.Libros WHERE LibroId=@LibroId AND Precio=0) BEGIN INSERT dbo.Biblioteca(UsuarioId,LibroId)VALUES(@UsuarioId,@LibroId);UPDATE dbo.Libros SET Descargas=Descargas+1 WHERE LibroId=@LibroId;SELECT 'library';RETURN;END IF EXISTS(SELECT 1 FROM dbo.Carrito WHERE UsuarioId=@UsuarioId AND LibroId=@LibroId) BEGIN SELECT 'already_cart';RETURN;END INSERT dbo.Carrito(UsuarioId,LibroId)VALUES(@UsuarioId,@LibroId);SELECT 'cart';END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Carrito_Contar @UsuarioId int AS BEGIN SET NOCOUNT ON;SELECT COUNT(*) FROM dbo.Carrito WHERE UsuarioId=@UsuarioId;END
GO
...
GO
CREATE OR ALTER PROCEDURE dbo.usp_Carrito_Comprar @UsuarioId int,@MetodoPago nvarchar(20),@ReferenciaPago nvarchar(180) AS
BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;IF @MetodoPago IS NULL OR @MetodoPago NOT IN(N'Card',N'PayPal') OR NULLIF(LTRIM(RTRIM(@ReferenciaPago)),N'') IS NULL THROW 50005,'Método de pago no válido.',1;IF EXISTS(SELECT 1 FROM dbo.Carrito c JOIN dbo.Libros l ON l.LibroId=c.LibroId WHERE c.UsuarioId=@UsuarioId AND l.AutorId=@UsuarioId) THROW 50008,'No puedes comprar tu propia publicación.',1;BEGIN TRAN;DECLARE @Total decimal(10,2)=(SELECT COALESCE(SUM(l.Precio),0) FROM dbo.Carrito c JOIN dbo.Libros l ON l.LibroId=c.LibroId WHERE c.UsuarioId=@UsuarioId);IF @Total=0 BEGIN ROLLBACK;RETURN;END DECLARE @Codigo varchar(40)=CONCAT('ORD-',FORMAT(GETDATE(),'yyyyMMddHHmmss'),'-',@UsuarioId,'-',LEFT(REPLACE(CONVERT(varchar(36),NEWID()),'-',''),8));INSERT dbo.Compras(Codigo,UsuarioId,Total,MetodoPago,ReferenciaPago)VALUES(@Codigo,@UsuarioId,@Total,@MetodoPago,@ReferenciaPago);DECLARE @CompraId int=SCOPE_IDENTITY();INSERT dbo.CompraDetalle(CompraId,LibroId,Precio) SELECT @CompraId,l.LibroId,l.Precio FROM dbo.Carrito c JOIN dbo.Libros l ON l.LibroId=c.LibroId WHERE c.UsuarioId=@UsuarioId;INSERT dbo.Biblioteca(UsuarioId,LibroId) SELECT @UsuarioId,c.LibroId FROM dbo.Carrito c WHERE c.UsuarioId=@UsuarioId AND NOT EXISTS(SELECT 1 FROM dbo.Biblioteca b WHERE b.UsuarioId=@UsuarioId AND b.LibroId=c.LibroId);UPDATE l SET Ventas=Ventas+1 FROM dbo.Libros l JOIN dbo.Carrito c ON c.LibroId=l.LibroId WHERE c.UsuarioId=@UsuarioId;DELETE dbo.Carrito WHERE UsuarioId=@UsuarioId;COMMIT;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Valoracion_Guardar @UsuarioId int,@LibroId int,@Puntuacion tinyint AS
BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;IF @Puntuacion NOT BETWEEN 1 AND 5 THROW 50009,'La puntuación debe estar entre 1 y 5.',1;IF EXISTS(SELECT 1 FROM dbo.Libros WHERE LibroId=@LibroId AND AutorId=@UsuarioId) THROW 50010,'No puedes valorar tu propia publicación.',1;IF NOT EXISTS(SELECT 1 FROM dbo.Biblioteca WHERE UsuarioId=@UsuarioId AND LibroId=@LibroId) THROW 50011,'El libro no pertenece a tu biblioteca.',1;BEGIN TRAN;UPDATE dbo.Valoraciones SET Puntuacion=@Puntuacion,Fecha=sysdatetime() WHERE UsuarioId=@UsuarioId AND LibroId=@LibroId;IF @@ROWCOUNT=0 INSERT dbo.Valoraciones(UsuarioId,LibroId,Puntuacion)VALUES(@UsuarioId,@LibroId,@Puntuacion);UPDATE dbo.Libros SET Valoracion=(SELECT CAST(AVG(CAST(Puntuacion AS decimal(4,2))) AS decimal(3,2)) FROM dbo.Valoraciones WHERE LibroId=@LibroId) WHERE LibroId=@LibroId;COMMIT;END
GO
...
GO
CREATE OR ALTER PROCEDURE dbo.usp_Usuario_Listar @Busqueda nvarchar(180)=NULL AS BEGIN SET NOCOUNT ON;SELECT u.UsuarioId,u.Nombre,u.Correo,r.Nombre Rol,u.EsAutor,COUNT(l.LibroId) Publicados,u.FechaRegistro Registrado,u.Activo FROM dbo.Usuarios u JOIN dbo.Roles r ON r.RolId=u.RolId LEFT JOIN dbo.Libros l ON l.AutorId=u.UsuarioId WHERE @Busqueda IS NULL OR u.Nombre LIKE '%'+@Busqueda+'%' OR u.Correo LIKE '%'+@Busqueda+'%' GROUP BY u.UsuarioId,u.Nombre,u.Correo,r.Nombre,u.EsAutor,u.FechaRegistro,u.Activo ORDER BY u.FechaRegistro;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Usuario_Estado @UsuarioId int,@Activo bit AS BEGIN SET NOCOUNT ON;IF EXISTS(SELECT 1 FROM dbo.Usuarios u JOIN dbo.Roles r ON r.RolId=u.RolId WHERE u.UsuarioId=@UsuarioId AND r.Nombre=N'Administrador') THROW 50012,'No se puede desactivar un administrador desde esta acción.',1;UPDATE dbo.Usuarios SET Activo=@Activo WHERE UsuarioId=@UsuarioId;IF @Activo=0 UPDATE dbo.Sesiones SET Fin=sysdatetime() WHERE UsuarioId=@UsuarioId AND Fin IS NULL;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Usuario_Actualizar @AdministradorId int,@UsuarioId int,@Nombre nvarchar(120),@Correo nvarchar(180),@Rol nvarchar(30),@Activo bit AS
BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;IF NULLIF(LTRIM(RTRIM(@Nombre)),N'') IS NULL OR NULLIF(LTRIM(RTRIM(@Correo)),N'') IS NULL THROW 50013,'Nombre y correo son obligatorios.',1;IF @Rol NOT IN(N'Administrador',N'Lector / Escritor') THROW 50014,'Rol no válido.',1;IF EXISTS(SELECT 1 FROM dbo.Usuarios WHERE Correo=@Correo AND UsuarioId<>@UsuarioId) THROW 50015,'El correo ya está registrado.',1;IF @UsuarioId=@AdministradorId AND (@Rol<>N'Administrador' OR @Activo=0) THROW 50016,'No puedes quitar tu propio acceso de administrador.',1;DECLARE @RolId int=(SELECT RolId FROM dbo.Roles WHERE Nombre=CASE WHEN @Rol=N'Administrador' THEN N'Administrador' ELSE N'Usuario' END);BEGIN TRAN;UPDATE dbo.Usuarios SET Nombre=LTRIM(RTRIM(@Nombre)),Correo=LTRIM(RTRIM(@Correo)),RolId=@RolId,EsAutor=CASE WHEN @Rol=N'Administrador' THEN 0 ELSE 1 END,Activo=@Activo WHERE UsuarioId=@UsuarioId;IF @UsuarioId<>@AdministradorId UPDATE dbo.Sesiones SET Fin=sysdatetime() WHERE UsuarioId=@UsuarioId AND Fin IS NULL;COMMIT;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Usuario_Crear @Nombre nvarchar(120),@Correo nvarchar(180),@Password nvarchar(100),@Rol nvarchar(30),@Activo bit=1 AS
BEGIN SET NOCOUNT ON;IF NULLIF(LTRIM(RTRIM(@Nombre)),N'') IS NULL OR NULLIF(LTRIM(RTRIM(@Correo)),N'') IS NULL THROW 50017,'Nombre y correo son obligatorios.',1;IF LEN(@Password)<6 THROW 50018,'La contraseña debe tener al menos 6 caracteres.',1;IF @Rol NOT IN(N'Administrador',N'Lector / Escritor') THROW 50019,'Rol no válido.',1;IF EXISTS(SELECT 1 FROM dbo.Usuarios WHERE Correo=LTRIM(RTRIM(@Correo))) THROW 50020,'El correo ya está registrado.',1;DECLARE @RolId int=(SELECT RolId FROM dbo.Roles WHERE Nombre=CASE WHEN @Rol=N'Administrador' THEN N'Administrador' ELSE N'Usuario' END);INSERT dbo.Usuarios(RolId,Nombre,Correo,PasswordHash,EsAutor,Activo)VALUES(@RolId,LTRIM(RTRIM(@Nombre)),LOWER(LTRIM(RTRIM(@Correo))),HASHBYTES('SHA2_256',CONVERT(nvarchar(4000),@Password)),CASE WHEN @Rol=N'Administrador' THEN 0 ELSE 1 END,@Activo);END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Sesion_Iniciar @UsuarioId int,@DireccionIp varchar(48)=NULL AS BEGIN SET NOCOUNT ON;INSERT dbo.Sesiones(UsuarioId,DireccionIp)VALUES(@UsuarioId,@DireccionIp);SELECT CONVERT(bigint,SCOPE_IDENTITY());END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Sesion_Cerrar @SesionId bigint AS BEGIN SET NOCOUNT ON;UPDATE dbo.Sesiones SET Fin=COALESCE(Fin,sysdatetime()) WHERE SesionId=@SesionId;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Sesion_ListarActivas AS BEGIN SET NOCOUNT ON;SELECT s.SesionId,u.UsuarioId,u.Nombre Usuario,u.Correo,CASE WHEN r.Nombre=N'Usuario' THEN N'Lector / Escritor' ELSE r.Nombre END Rol,s.Inicio,s.DireccionIp FROM dbo.Sesiones s JOIN dbo.Usuarios u ON u.UsuarioId=s.UsuarioId JOIN dbo.Roles r ON r.RolId=u.RolId WHERE s.Fin IS NULL AND u.Activo=1 ORDER BY s.Inicio DESC;END
GO
CREATE OR ALTER PROCEDURE dbo.usp_Sesion_Validar @SesionId bigint,@UsuarioId int AS BEGIN SET NOCOUNT ON;SELECT CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.Sesiones s JOIN dbo.Usuarios u ON u.UsuarioId=s.UsuarioId WHERE s.SesionId=@SesionId AND s.UsuarioId=@UsuarioId AND s.Fin IS NULL AND u.Activo=1) THEN 1 ELSE 0 END AS bit);END
GO
...
BEGIN SET NOCOUNT ON;DECLARE @Genero nvarchar(80),@Paginas nvarchar(40),@Idioma nvarchar(40),@Formato nvarchar(40),@Ritmo nvarchar(40),@Ambiente nvarchar(40),@Descubrimiento nvarchar(40);SELECT @Genero=Genero,@Paginas=Paginas,@Idioma=Idioma,@Formato=Formato,@Ritmo=Ritmo,@Ambiente=Ambiente,@Descubrimiento=Descubrimiento FROM dbo.PreferenciasUsuario WHERE UsuarioId=@UsuarioId;
 SELECT l.LibroId,l.Codigo,l.Titulo,u.Nombre Autor,g.Nombre Genero,l.Idioma,l.Descripcion,l.Precio,l.Valoracion,l.PortadaUrl,l.Estado,l.Ventas,l.Descargas,CAST(0 AS bit) Leido,CAST(l.FechaPublicacion AS datetime2) Fecha,CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.Biblioteca b WHERE b.UsuarioId=@UsuarioId AND b.LibroId=l.LibroId) THEN 1 ELSE 0 END AS bit) EnBiblioteca,CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.Carrito c WHERE c.UsuarioId=@UsuarioId AND c.LibroId=l.LibroId) THEN 1 ELSE 0 END AS bit) EnCarrito,CAST(CASE WHEN l.AutorId=@UsuarioId THEN 1 ELSE 0 END AS bit) EsPropio,(SELECT COUNT(*) FROM dbo.Valoraciones v WHERE v.LibroId=l.LibroId) CantidadValoraciones,
